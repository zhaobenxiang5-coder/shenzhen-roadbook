import json
from generate_star_detailed_roadbook import ENRICHED_DAYS
from generate_all_in_one_roadbook import FULL_ROADBOOK
from generate_super_app import CHECKLIST_CATEGORIES

# Combine Mid-Autumn enriched and National Day enriched
MERGED_DAYS = []
for day in FULL_ROADBOOK:
    # Check if Mid-Autumn has enriched schedule
    found_mid = next((d for d in ENRICHED_DAYS if d["day_id"] == day["day_id"]), None)
    if found_mid:
        MERGED_DAYS.append(found_mid)
    else:
...
<head>
  ...
  <meta property="og:description" content="情侣专属·厌人避堵·每日回丹平睡大床·A/B/C三重平替·小清新慢调吃玩全清单 ✨">
  ...
  <style>
    ...
    * {{
    ...
    @keyframes twinkle {{
      ...
      50% {{ opacity: 0.85; transform: scale(1.15) rotate(45deg); }}
...
for day in MERGED_DAYS:
    ...
    html += f"""
      <article class="day-card" id="{day_id}">
        <div class="day-header">
          <span class="day-title"><span class="sparkle">✨</span> {day['date_title']}</span>
    ...
    for p_key, p_color in [("a", "#fb923c"), ("b", "#34d399"), ("c", "#a78bfa")]:
        ...
        if plan.get("schedule"):
            ...
            for step in plan["schedule"]:
                ...
                <div class="schedule-step">
                  ...
                  <div class="schedule-act">{step['action']}</div>
                </div>
                """
            schedule_html = f"""
  ...
  <script>
    ...
    function saveCheckState() {{
      try {{
        ...
        localStorage.setItem('roadbook_checklist_starry_v5', JSON.stringify(states));
      }} catch (e) {{}}
    }}

    window.addEventListener('DOMContentLoaded', () => {{
      try {{
        const saved = JSON.parse(localStorage.getItem('roadbook_checklist_starry_v5'));
        if (saved) {{
          const checkboxes = document.querySelectorAll('.check-item input[type="checkbox"]');
...
print("Starry super roadbook generated successfully!")
